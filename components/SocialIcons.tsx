...
const item = {
  ...
  show: { y: 0, opacity: 1, transition: { ease: [0.22, 1, 0.36, 1] } },
};
    ...
    >
      {socials.map((social) => (
        ...
        >
          {social.icon}
          {/* Hover glow */}
          <span className="absolute inset-0 rounded-full bg-accent/0 group-hover:bg-accent/10 blur-sm transition-all duration-300" />
        </motion.a>
      ))}
    </motion.div>
  );
